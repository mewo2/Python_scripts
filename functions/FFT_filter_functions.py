...
def FFT_max_value_bearings_NORTH_FRQ_LESS_180(pos_x_coord,pos_y_coord,frq):
	frq_float = frq + 0.0
	origin_x = frq_float
	origin_y = frq_float
	north_x = frq_float
	north_y = frq_float*2 	
	a = math.sqrt((abs(origin_x - pos_x_coord)**2)+(abs(origin_y - pos_y_coord)**2))
	b = frq_float
	c = math.sqrt((abs(north_x - pos_x_coord)**2)+(abs(north_y - pos_y_coord)**2))
	cosC = (((a**2) + (b**2)) - (c**2)) // (2*((a)*(b)))
	#C_rad = math.acos(cosC)
...
def FFT_max_filter_values(freq, post, input_x, input_y, magnitude, kernel=50):
		...
		for POI in range(len(max_coord_x)):
			...
			f.write("%f,%f,%f,%f" %(mag,instance_dist_frq,instance_dist_px,instance_dist_px*post))
			f.write("\n")
