...
image_array_subsample_DATA = image_array
...
imgout = [np.zeros((nxwind // stepsize, nywind // stepsize)) for i in xrange(nvars)]
window_iteration=0
...
for vv in range(nvars):
	...
	time_stamp = strftime("%H.%M.%S")
	image_output_centre_cell = r'/geog/data/arcturus/epsilon/ggwillc/vario_outputs/' + str(vv+1) + '_centre_cell_output_%s_RANDOM.png' %(time_stamp)
	#plt.show()
